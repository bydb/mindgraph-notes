...
const DashboardOfferCard: React.FC<{ offer: EdooboxOfferDashboard }> = ({ offer }) => {
  ...
  return (
    <div className={`agent-dashboard-card panel-card ${expanded ? 'expanded' : ''}`}>
    ...
    <>
      {events.length === 0 ? (
        ...
        <div className="agent-event-list">
          {events.map(event => (
            <button key={event.id} className="agent-event-item panel-card" onClick={() => setSelectedEventId(event.id)}>
...
const MarketingOfferCard: React.FC<{ offer: EdooboxOfferDashboard; onSelect: () => void }> = ({ offer, onSelect }) => {
  ...
  return (
    <button className="agent-marketing-card panel-card" onClick={onSelect}>
...
const IqOfferCard: React.FC<{ offer: EdooboxOfferDashboard; onSelect: () => void }> = ({ offer, onSelect }) => {
  ...
  return (
    <div className="agent-dashboard-card panel-card">
      <button className="agent-dashboard-card-header" onClick={onSelect}>
        <div className="agent-dashboard-card-body">
          <div className="agent-dashboard-card-title">{offer.name}</div>
          <div className="agent-dashboard-card-meta">
            {offer.dateStart && <span>{formatDate(offer.dateStart)}</span>}
        ...
        <svg className="agent-dashboard-chevron" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <path d="m9 18 6-6-6-6" />
        </svg>
      </button>
    </div>
