...
describe('Modellnamen-Kanonisierung (LM Studio ↔ Ollama)', () => {
  ...
  it('supportsNativeToolCalls funktioniert für beide Namensformen', () => {
    ...
    expect(supportsNativeToolCalls('google/gemma-4-12b-it')).toBe(true)
    expect(supportsNativeToolCalls('gemma4:latest')).toBe(true)
    expect(supportsNativeToolCalls('glm-5:cloud')).toBe(true)
    expect(supportsNativeToolCalls('kimi-k2.5:cloud')).toBe(true)
    expect(supportsNativeToolCalls('glm-ocr:latest')).toBe(true)
    expect(supportsNativeToolCalls('dengcao/Qwen3-Reranker-0.6B:Q8_0')).toBe(false)
    expect(supportsNativeToolCalls('olmo-3:7b-think-q8_0')).toBe(false)
  })
})
