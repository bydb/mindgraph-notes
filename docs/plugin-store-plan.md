...
- **Phase A-pre — Kern entkennt die Plugins (aus Codex-Review, Voraussetzung).** (1) ✅ Modulschalter `enable/disable` → echtes `registry.activate/deactivate`; `activateAll` respektiert den Disabled-Zustand. (2) ✅ Modul-Metadaten + Schalter-Liste **generisch aus den Manifesten** statt `uiStore.MODULES`/`modules.ts`-Switch. (3) ⏳ Plugin-Settings aus `Settings.tsx` → plugin-contributed Settings-Slots; Plugin-Config generisch (keyed by pluginId) statt `state.antares.*` im uiStore — **Antares migriert; edoobox/remarkable folgen**. (4) ✅ Antares-/edoobox-Workflow**logik** aus `workflowStore` in die Vertikalen: neuer `WorkflowTriggerProvider`-Vertrag (`shared/plugins/workflowTrigger.ts`), Renderer-Slot `workflow.trigger`, Provider in `src/plugins/{antares,edoobox}/renderer/workflowTrigger.ts` (Formatierung, Item-Bau, Ledger-Reset/Baseline); `workflowStore` dispatcht generisch (kein Plugin-Import, keine Plugin-Trigger-IDs mehr, Ledger-Eviction wert- statt namensbasiert). Der Event-Dispatch liegt in `renderer/stores/workflowTriggerDispatch.ts` (DI, store-frei → pur testbar). **Bugfix dabei:** `afterRun` (edoobox-Baseline) rückt jetzt nur bei **vollständig erfolgreichem** Lauf vor — vorher rückte die Baseline auch bei `status:'failed'`-Läufen vor (kein throw) und ließ die Anmeldung verloren gehen. Tests: Antares-Reset, edoobox-Erstbaseline/Delta, Fehlschlag-Gate (+Teilfehler), Leerlauf-Init, Exactly-once-Filter. **Rest-Kopplung (Metadaten-Ebene): die Trigger-Bausteine `antares.mahnung`/`edoobox.newBooking` stehen noch hart in `shared/workflow/registry.ts` (Palette) + `main/workflows/runner.ts:172` (generischer Executor) → Palette-Block bliebe nach Ordner-Löschung sichtbar (Lauf skippt aber sauber). Nächster Sub-Schritt: Workflow-Actions plugin-contributed registrieren.** (5) ✅ Readiness-Check pro Plugin (Credentials/Config vorhanden?). **Akzeptanz: echter automatisierter Deletion-Test** — Ordner weg ⇒ keine toten Settings/Schalter/Workflow-Zweige. Danach erst ist „restlos weg" wahr.
- **Phase A0 — Verträge & Format.** `@mindgraph/plugin-api` schälen (aus heutigem `shared/plugins/*`), Manifest v2 definieren, Repo-Template + Build/Sign-Action bauen. Noch kein App-Verhalten geändert.
