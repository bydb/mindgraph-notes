...
Der Deckel (1/2/3) ändert bei K8 nichts. Die drei Negativfälle, die auch bei 0,50 beantwortet werden, liegen thematisch nah am Vault (MindGraph-Funktion, die es nicht gibt; Telefonnummer eines im Vault nur beiläufig erwähnten Betriebs; Aussage einer Person zur Digitalwoche) mit besten Scores 0,55–0,66 — dort kann kein Floor trennen, dort muss die Antwort „steht nicht in den Quellen“ sagen (Handbewertung mit `--answer`). Vier Positivfälle ohne Treffer: zwei Fragen zu derselben Personalnotiz (Rang 7 bzw. Kandidatenrang 26), zwei mit Kandidatenrang über 100 (Frage nach dem „Warum Pico“, Frage nach einer in einer Mail genannten Person) — Kandidaten für den Wortabgleich.
...
**Handbewertung durch den Nutzer (20.09.2026):** 21 von 54 Fällen bewertet — 17 richtig, 2 teilweise (die zweite Fassung der Personalnotiz-Frage; der Negativfall „Telefonnummer“, bei dem das Modell korrekt verweigert, aber fremde Nummern erwähnt), 2 richtig verweigert; **kein Fall falsch, kein „falsch grün“, kein „falsch rot“.** Darunter die acht Fälle mit den meisten harten Auffälligkeiten (Sätze ohne Quelle, Zitate nicht im Original), geprüft per Klick auf die Quelle. Rückmeldung des Nutzers: Die Antworten stimmten, soweit beurteilbar; die von Claude aus Notizausschnitten erzeugten Fragen wirkten fremd („so würde ich keine Fragen stellen“), und 54 Fälle übersteigen die Konzentration für eine Handbewertung — Bewertungsmüdigkeit („hört sich alles richtig an“) ist ein realer Messfehler. **Konsequenz:** Holdout-Set nur aus selbst formulierten Fragen des Nutzers (12 positiv, 6 negativ), Handbewertung künftig nur für markierte Sätze.

**Entscheidung (vorläufig gemessen, unabhängige Bestätigung offen — 20.09.2026):** Floor 0,50 und seltenheitsgewichteter Wortabgleich mit Gewicht 0,3 sind als Standard in der App (`vaultRetrieve.ts`, `shared/rag/lexical.ts`); das Eval-Skript reproduziert mit den App-Standards 39/42, MRR 0,78, 9/12 verweigert. Prompt-Regel für Anführungszeichen geschärft (nur Wort-für-Wort-Übernahmen), Wirkung auf „Zitate nicht im Original“ (6 von 187 Sätzen) wird am Holdout gemessen.

**Nutzerfall 20.09., 11:44 („Findest du etwas … zum Thema Computer use“, vom Nutzer als sehr gut bewertet):** ehrliche Antwort („nicht als eigene Notiz vorhanden, aber …“ mit sechs belegten Sätzen). Die Prüfzeile meldete zwei „Zitate nicht im Original“ — beide waren Nennungen: der Suchbegriff aus der Frage („Computer use“) und ein Notiztitel („Lokale Modelle in MindGraph Notes“) in Anführungszeichen. Das war „falsch rot“ durch die Prüfung selbst. Behoben: in Anführungszeichen gesetzte Begriffe, die in der Frage vorkommen oder einem Quellentitel/-dateinamen entsprechen, gelten als Nennung und werden nicht als Zitat geführt (`citations.ts`, Optionen `question`/`sourceTitles`, Test). Nebenbefund: vier der acht Quellen waren nahezu gleiche Kopien einer Projektstatus-Notiz (`_STATUS-2026-W21`, `W21 (2)`, `W20 (6)`, `W22`) — Dedupe ist nur exakt; Near-Duplikate über Dateien hinweg bleiben ein Phase-3-Punkt.

**Holdout (erste sechs vom Nutzer selbst formulierte Fragen, 20.09.2026, App-Standards Floor 0,50 + Wortabgleich 0,3):** vier Themenfragen ohne feste Erwartungsdatei (Trainingslehre Enduro, Relevanz Zettelkasten, 10 000-Euro-Erlass, Handschrift in Schulen) alle beantwortet mit passenden Quellen (Erlass-Notiz, Zettelkasten-/POS-Notizen, Handschrift-Recherche); die Enduro-Frage ehrlich als „nur Bewegungslernen am Beispiel Mountainbike, keine Trainingslehre“ eingeordnet. Beide Negativfälle enden in einer Verweigerung: „Ukraine-Krieg“ unter dem Floor, „Schwimmtechnik“ (bester Score 0,52) vom Modell verweigert. Prüfzeilen: 21 Sätze, 2 ohne Quelle, 3 niedrige Deckung, 0 ungültig, 1 Zitat nicht im Original. Handbewertung durch den Nutzer: **alle sechs richtig** (vier Antworten richtig, „Schwimmtechnik“ richtig verweigert durch das Modell, „Ukraine“ richtig verweigert durch den Floor). **Das ist ein Pilot, keine unabhängige Bestätigung** (Codex F44): sechs Fälle tragen keine zwei Hyperparameter, und dasselbe Set wurde danach beim Feinschliff erneut ausgewertet — es ist damit ein zweites Tuning-Set. Floor 0,50 und Gewicht 0,3 gelten deshalb als **vorläufig gemessene Standardwerte**. Ein neues, ungesehenes Set natürlicher Nutzerfragen (mit Verneinungen, kurzen Fragen, Namen, Zahlen, Synonymen und Synthesefragen) wird gesammelt und genau einmal ausgewertet. Erneut sichtbar: nahezu gleiche Kopien einer Notiz (`10000 Euro Erlass` und `… - Ueberprueft`) belegen mehrere Quellenplätze.
...
- **Skills-Ordner** (Agent-Anweisungen und Beispielmaterial) nie indexieren; **Ladehinweis** statt „Vorbereitung“ während der 42 s Modellprobe.
- **Suche in der Notiz (⌘F) im Lesen-Modus** ergänzt (`utils/inNoteSearch.ts`) — beim Durchklicken der Bewertungsprotokolle fiel auf, dass es sie nicht gab. Markierung über die CSS-Highlight-Schnittstelle, kein DOM-Eingriff (das WYSIWYG-Autosave hätte `<mark>`-Elemente in die Notiz geschrieben); im GUI geprüft: 5 Treffer, Weiter/Enter/Escape, Notiz-Hash unverändert.
- Offen für Teil 3: Filterleiste im Vault-Modus (bewusst zurückgestellt), dauerhafte Konkurrenz-/Lebenszyklus-Tests, CHANGELOG in Nutzerfassung.

### Phase-3-Review (Codex, 20.09.2026) — Runde 9 umgesetzt

- **F42 Policy-Migration.** Die Indexregeln haben jetzt eine Version (`INDEX_POLICY_VERSION`) in der Index-Identität, aber nicht in `embeddingsCompatible`: ein Altindex gilt als veraltet (sichtbarer Hinweis in der Einstellungs-Karte mit Knopf „Neu aufbauen“), die vorhandenen Einbettungen werden aber weiterverwendet — der Neuaufbau dauert Sekunden statt Minuten. Weicht die Version ab, erzwingt der Indexer einen Rescan; der inkrementelle Schnellpfad darf keine Datei mehr ungelesen übernehmen. Zusätzlich zwei Netze für bestehende Installationen: `isIndexable` filtert bei JEDER Abfrage (vorher nur bei gesetzten Nutzer-Ausschlüssen), und die Frischeprüfung wirft abgeleitete KI-Notizen am frischen Inhalt weg. Regressionstest mit Gegenprobe: ohne die Migration fällt er.
- **F43 Eval misst den Produktpfad.** Auswahl in einer Funktion (`selectForQuery`), die App und Harness gemeinsam nutzen: Kandidatentiefe topK × oversample, Floor, Wortabgleich, Dedupe/Deckel, `score` bleibt die Bedeutungsnähe. Das Harness holt 100 Kandidaten nur noch für die Diagnose `candidatenrang`. Paritätstests: ein Treffer jenseits der Tiefe gelangt nicht in die Auswahl; innerhalb hebt der Wortabgleich ihn nach vorn. **Neu gemessen am Produktpfad: 39/42, MRR 0,81** (vorher veröffentlicht 39/42, MRR 0,78 — die Trefferquote hält, die Rangfolge ist etwas besser als behauptet).
- **F45 `familyKey` eng gefasst.** Nur noch das maschinelle Kopie-Muster „Notiz (2)“. Endungen wie „- alt“, „- final“, „- v2“, „- Ueberprueft“ sind vom Nutzer vergebene Namen für eigenständige Fassungen; da die zweite Datei einer Familie vollständig verworfen wird, hätte die Heuristik einen legitimen Entwurf unsichtbar gemacht.
- **F44 Holdout** als Pilot ausgewiesen, Standardwerte als vorläufig gemessen (siehe oben).
- **F46 Nennungs-Ausnahme: Frage-Ausnahme ersatzlos entfernt** (Runde 10). Die erste Fassung ließ jeden Ausdruck bis fünf Wörter durch, sobald er in der Frage vorkam — ein erfundenes Kurzzitat wäre unbeanstandet geblieben. Geblieben ist nur: Ausdruck gleich Titel einer gelieferten Quelle.
- **F47 Suche in der Notiz.** Ein Beobachter sucht nach jedem Umbau der Vorschau (nachgeladene Bilder, Faltungen) neu; vorher zeigten die Ranges auf abgehängte Knoten und die Markierung verschwand lautlos.
- **F48 offen und so ausgewiesen:** Die `expected`-Angaben im Eval sind Teilstrings des Dateipfads, und die Fragen liegen im Vault des Nutzers. Beides ist bewusst so (private Daten gehören nicht ins Repo), macht die Zahlen aber nicht von außen nachvollziehbar. Für die nächste Messung: erwartete Dateien exakt benennen und das Set anonymisiert ablegen.
...
Beispieltreffer einer Abfrage nach dem Inhalt eines zurückliegenden Termins: eine Inbox-Notiz, eine Archiv-Notiz eines Termins, zwei Brain-Tage und eine Mail-Notiz, Scores 0,55–0,56, alle als frisch geprüft. Die Scores liegen eng beieinander; die Rangfolge an der Spitze ist genau das, was Phase 3 misst.
...
### 2. Lebenszyklus, Privacy/Injection, Export — GUI-Abnahme 19.09.2026 (Dev-App, echter Vault und Modelltest-Vault)

**Lebenszyklus (2026-Vault, 4547 Notizen):**
- **Erstaufbau:** scanning → embedding (Pakete à 20) → Checkpoint pro Paket in `userData/rag-staging/<hash>/`. Vor dem Embedding steht „Modell prüfen“ **42 s** lang: das ist die Probe-Einbettung, mit der Ollama `bge-m3` neben dem residenten 27B-Chatmodell lädt (headless ohne Chatmodell: unter 1 s). Kein Fehler, aber 42 s ohne Fortschrittsänderung — Hinweis für Phase 3 (Text „Embedding-Modell wird geladen“).
...
- **Vault-Wechsel** (nur über den Systemdialog, zweimal mit dem Nutzer durchgeführt): die Einstellungs-Karte des neuen Vaults zeigte keinen Fortschritt des alten (vault-gebundene Ereignisse), Status des alten Vaults ohne Job. Eine im alten Vault beantwortete Vault-Frage bleibt im Chat sichtbar, ist aber inert: Hochzahlen und Quellenzeilen tragen keine Notiz-ID mehr, ein Klick öffnet nichts. **Wechsel während einer laufenden Antwort** (Codex F41, dritter Versuch mit durch Modell-Entladen auf eine Minute verlängertem Fenster): nach 112 gestreamten Chunks gewechselt → Main meldete `cancelled`, keine verspätete Nachricht, kein Streaming-Cursor, Eingabe und Senden sofort frei. Vorher blieben Streaming-Zustand und Antwortpuffer stehen (Eingabe gesperrt); der Lebenszyklus der Vault-Anfrage liegt jetzt in `renderer/utils/vaultRequest.ts` mit Tests. **Wechsel exakt in der Startphase des Index-Aufbaus** (F30, mit Codex' Vorschlag reproduzierbar gemacht: Umgebungsvariable `MINDGRAPH_VAULT_RAG_START_DELAY_MS` verlängert im Manager die Startphase vor der Jobzuweisung, ohne Variable wirkungslos): Dev-App mit 60 s Verzögerung, „Neu aufbauen“ im Modelltest-Vault, nach rund 25 s Wechsel nach 2026 → kein Job wurde zugewiesen, keine Fortschrittsereignisse, beide Vaults ohne Lauf, 2026 mit intaktem Index. Der Start war verworfen, wie es die Manager-Tests beschreiben.
...
- **Verlauf nach Anbieterwechsel:** reine Funktion `chatHistory.ts` mit Tests (Vault-Frage und -Antwort nie im Verlauf, Begrenzung nach dem Ausschluss, nur Rolle und Inhalt).
- **Remote-Modell / manipulierte IPC-Aufrufe:** `vault-rag-answer` nimmt kein Modell vom Renderer (Signatur `vaultPath, query, requestId, language`); Chat-Modell aus `ui-settings.json` durch `resolveLocalModel` (Cloud-Suffix, `remote_model`/`remote_host`, mehrdeutig → Ablehnung, `localModel.test.ts`); Einbettung der Frage durch dieselbe Sperre in `embed.ts` (`embed.test.ts`). Prüfung vor dem Senden, nicht danach.

**Quellen-Roundtrip (Modelltest-Vault, Notizen `A/Messreihe.md` und `B/Messreihe.md` mit gleichem Dateinamen):**
- Antwort zitiert `[1]` (A) und `[2]` (B) mit zwei verschiedenen Notiz-IDs; Klick öffnet jeweils die richtige Notiz (Kopfzeile „A / …“ bzw. „B / …“), Hervorhebung auf der Überschrift.
- **Gefunden und behoben:** beim Wechsel von Notiz A zu B per Klick trug `previewContent` für einen Render noch den Text von A unter der ID von B — falscher Hinweis „Quelle geändert“ und Sprung nach oben. Jetzt gilt eine Hash-Abweichung erst nach 1,5 s Karenz als „geändert“; passt der Inhalt vorher, wird gesprungen. Danach: A → B → A ohne Hinweis, Status `ok` nach ~190 ms.
- **Lesen, Schreiben, Markdown:** Lesen hebt hervor; Schreiben-Modus (`live-preview`) setzt Cursor und Sichtbereich auf die Zeile (CodeMirror fokussiert, aktive Zeile „# Messreihe Ordner A“); Markdown-Modus war in Runde 4/5 belegt. Öffnet ein Klick eine andere Notiz, startet sie wie üblich im Lesen-Modus.
- **Export:** „Kopieren“ liefert Antwort + Fußnoten `[^q…-n]` mit vollem Pfad und Prüfzeile; „An aktuelle Notiz anhängen“ zweimal → zwei Blöcke mit verschiedenen IDs (`qmu8b78fuq4c`, `qmu8b7ag3cnt`), eigene Fußnoten-Definitionen, Provenienz-Callout; „Als neue Notiz speichern“ → Notiz im Standard-Notizordner mit eigener ID. **Gefunden und behoben:** existierte der konfigurierte Standard-Notizordner im aktuellen Vault nicht (Ordner eines anderen Vaults), scheiterte die Übernahme mit „Übernahme in den Vault fehlgeschlagen“, weil die Kollisionsprüfung vor dem Anlegen des Ordners lief; der Ordner wird jetzt zuerst angelegt.
- **Fehlende Datei** (temporäre Testnotiz gelöscht): nach dem Watcher-Update zeigt der Fußbereich die Quelle als Text ohne Link, die Hochzahl ist nicht mehr klickbar — der Klick endet sicher, nichts öffnet sich. Der Zwischenzustand (Datei weg, Notiz noch im Store) liefert den Hinweis „Quelle nicht mehr vorhanden“ (`sourceJump.test.ts`, `vaultRetrieve.test.ts`).
...
- Alle in F38 verlangten GUI-Fälle sind damit im GUI belegt (Startphasen-Wechsel seit Runde 8 über die Verzögerungs-Variable).
...
- **Aus dem ersten Nutzertest nach der Abnahme (20.09.2026):** Der Ausschluss „400 - 🏛️ Archiv“ (Emoji mit Variantenselektor U+FE0F) traf den Ordner „400 - 🏛 Archiv“ (ohne) nie — das Archiv war trotz Einstellung im Index, eine Archivnotiz von 2020 tauchte als Quelle auf. Ordner-Abgleiche (Ausschluss, Abfragefilter, Identitätsschlüssel) laufen jetzt in einer Vergleichsform (NFC, ohne Variantenselektoren); gespeicherte Pfade bleiben unverändert. `node_modules` ist wie Vorlagen-Ordner immer ausgeschlossen (ein `CHANGELOG.md` aus einem Projektordner war Quelle). Beobachtung für die Messung: eine Frage ohne den Namen der Stelle fand die Notiz zur Stellenbewertung nicht unter den besten acht (Rang außerhalb der Top 20), mit dem Namen auf Rang 1 — Retrieval-Qualität ist Phase 3.
