...
## Erweiterung: Ordner als Kontext (Konzept 2026-07-04, Stufen 1+2 umgesetzt)
...
2. **Selektives Lesen im Agent-Loop (Phase 2) — UMGESETZT (2026-08-04):** derselbe Ordner-Anhang wird zur Skill-Fläche. Umgesetzt als `list_context_folder(folder)` (Manifest ohne Inhalte, bei Excel inkl. Blattnamen/Zeilenzahl; Blatt-Details nur für die ersten 40 Dateien, der Rest steht als ausgelassen im Manifest) + `read_context_file(folder, file, sheet?, offset?, max_rows?)` (eine Datei, blatt- und zeilenweise; beim Blättern wird die Kopfzeile vorangestellt). Freigeschaltet nur bei vorhandenem Ordner-Anhang; Iterations-Budget solcher Läufe 20 statt 12.
   **Vorverdichtung für große Tabellen-Ordner (2026-08-04):** `collect_table(folder, columns, sheet?, filter?, files?)` führt ALLE Excel-/CSV-Dateien Main-seitig zusammen (Kopfzeilen-Erkennung, unscharfe Spaltenzuordnung, Herkunftsspalte „Quelldatei", Filter `nicht_leer|enthaelt|gleich|datum_zwischen`) und legt das Ergebnis als Datensatz im Lauf ab — das Modell bekommt nur Kennzahlen, 20 Beispielzeilen und die Problemliste. `write_xlsx` nimmt statt `rows` den `dataset`-Verweis und schreibt zusätzlich ein Blatt „Nicht verwertet"; `peek_dataset` liefert Zeilen portionsweise, wenn doch inhaltlich beurteilt werden muss. Grenzen: 300 Dateien, 20.000 Zeilen, beides sichtbar gemeldet. Pure Logik + Tests: `shared/tableCollect.ts` / `.test.ts`. Damit laufen die Zeilen nie durch den Modellkontext — der stille Überlauf (siehe `shared/contextGuard.ts`) kann das Ergebnis nicht verfälschen.
