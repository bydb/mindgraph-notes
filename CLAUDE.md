...
- **Indexer** (`main/rag/vaultIndexer.ts`): Pakete à 20 Dateien = Checkpoint-Einheit; **Pause wird vor jedem Embedding-Request geprüft** (`ollamaActivity.ts`, Vordergrund = Notiz-Chat, Projekt-RAG-Antwort, Vault-Abfrage, Mail-Analyse, Brain, Notiz-Agent; NICHT Telegram/Workflow/Smart Connections). Beginnt Vordergrund, werden laufende Requests abgebrochen und später wiederholt. Staging-Segmente + Checkpoint in `userData/rag-staging/`; Resume gleicht per Hash ab (geändert → neu, gelöscht → raus). Digest vor **und nach** Build sowie vor und nach jeder Frage-Einbettung.
